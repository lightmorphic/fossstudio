...
export function recipe(audioIds, videoIds) {
  return {
    audio: AUDIO_FORMATS.filter((f) => audioIds.includes(f.id))
      .map((f) => ({ id: f.id, mimes: f.mimes })),
    video: VIDEO_FORMATS.filter((f) => videoIds.includes(f.id))
      .map((f) => ({ id: f.id, mimes: f.mimes, camMimes: f.camMimes }))
  };
}
